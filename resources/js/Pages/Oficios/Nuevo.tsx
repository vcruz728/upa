...
export default function Nuevo({
    ...
    const postUno = (payload: {
        ...
        new Promise<void>((resolve, reject) => {
            router.post(route("oficios.saveDestinatarioNuevo"), payload, {
                preserveScroll: true,
    ...
    const guardarDestinatarios = async () => {
        ...
        const payloads = seleccionados.map((d) => ({
        ...
        for (const p of payloads) {
            try {
                // 👇 secuencial: espera a que termine antes de enviar el siguiente
                await postUno(p);
                ok++;
            } catch {
                fail++;
        ...
        if (ok) toast.success(`Guardados ${ok} destinatario(s).`);
        if (fail) toast.error(`No se pudieron guardar ${fail}.`);

        if (ok && !fail) destRef.current?.clearSelected();
    };
    const {
        data,
        setData,
