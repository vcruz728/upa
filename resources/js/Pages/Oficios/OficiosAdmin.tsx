...
export default function MisOficios({
    ...
    // Guarda/recupera estado de DataTables (incluye página)
    const dtStateSave = (key: string) => ({
        ...
        stateSaveCallback: (settings: any, data: any) => {
            try {
                const api = new (jQuery as any).fn.dataTable.Api(settings);
                const info = api.page.info();
                data.start = info.start; // índice de inicio (página)
                data.length = info.length; // tamaño de página
            } catch {}
        ...
        // TRAS INICIALIZAR, IR A LA PÁGINA GUARDADA (si existe)
        initComplete: function (this: any) {
            const api = this.api();
            const state = api.state ? api.state() : null;
            if (!state) return;

            const info = api.page.info();
            const len = state.length ?? info.length ?? 25;
            const desiredStart = state.start ?? 0;
            const desiredPage = Math.floor(desiredStart / len);

            // si el dataset cambió y la página ya no existe, recorta a la última
            const lastPage = Math.max(
                0,
                Math.ceil(info.recordsDisplay / len) - 1
            );
            api.page(Math.min(desiredPage, lastPage)).draw(false);
        },
    });
                            ...
                            <Card.Body>
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        {/* TAB 5: OFICIOSUPA– HISTÓRICO */}
                                        <Tab
                                            eventKey="tab5"
                                            title="OficiosUPA- Histórico"
                                        >
                                            <Col md={12}>
                                                <div className="mb-3 d-flex justify-content-end">
                                                    <button
